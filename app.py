                ...
                with st.spinner('Summarization ...'):
                    ...
                    if st.button("Summarize",disabled=st.session_state.disabled):
                        if st.session_state.llm == "Open-AI":
                            ...
                            memory = memory,
                            verbose=True)
                            st.session_state["tmp_summary_gpt_aml"] = conversation.predict(input="Provide a detailed summary of the text provided in a single paragraph. Please don't include words like these: 'chat summary', 'includes information', 'AI analyzes' in my final summary."")
